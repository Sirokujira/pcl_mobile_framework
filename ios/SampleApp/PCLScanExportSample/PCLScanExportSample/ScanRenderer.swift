...
final class ScanRenderer: NSObject, MTKViewDelegate {
    ...
    private unowned let coordinator: ScanExportCoordinator
    ...
    init(coordinator: ScanExportCoordinator) {
    ...
    func draw(in view: MTKView) {
        ...
        if let (buf, count) = coordinator.currentCloud() {
            let mvp = makeMVP(viewportSize: view.drawableSize)
            var u = ScanUniforms(mvp: mvp, pointSize: 3.0, maxDist: 5.0)
            memcpy(uniBuf.contents(), &u, MemoryLayout<ScanUniforms>.size)
    ...
    private func makeMVP(viewportSize: CGSize) -> simd_float4x4 {
        ...
        let proj   = perspFov(fovY: 60 * .pi/180, aspect: aspect, near: 0.01, far: 200)
        let e = eye()
    ...
    private func buildPipeline() {
        ...
        d.vertexFunction                  = lib.makeFunction(name: "scanPointVertex")
        d.fragmentFunction                = lib.makeFunction(name: "scanCircleFragment")
        ...
        uniBuf     = device.makeBuffer(length: MemoryLayout<ScanUniforms>.size, options: .storageModeShared)
    }
}
